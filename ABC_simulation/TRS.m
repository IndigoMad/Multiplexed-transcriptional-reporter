function[A,B,C,Ac,Bc,Cc,Apc,Bpc,Cpc,btA,btB,btC,bsA,bsB,bsC]=TRS(kon1A,konA,koffA,ktA,kpA,kdmA,kdpA,kon1B,kaB,nB,konB,koffB,ktB,kpB,kdmB,kdpB,kon1C,kaC,nC,konC,koffC,ktC,kpC,kdmC,kdpC,T,tau)
...
for t=1:T
    ...
    %=================Gene B==================
    %=========================================
    kon1Btem=kon1B*(Apc(t)^nB/(Apc(t)^nB+kaB^nB));
    pon1B=1-exp(-kon1Btem);
    konBtem=konB*(Apc(t)^nB/(Apc(t)^nB+kaB^nB));
    ponB=1-exp(-konBtem);
    %=================B================
    %==========%Additional State========
    if onB==(-1)
    ...
    %=================Gene C==================
    %=========================================
    kon1Ctem=kon1C*(Bpc(t)^nC/(Bpc(t)^nC+kaC^nC));
    pon1C=1-exp(-kon1Ctem);
    konCtem=konC*(Bpc(t)^nC/(Bpc(t)^nC+kaC^nC));
    ponC=1-exp(-konCtem);
    %=================C================
    %==========%Additional State========
    if onC==(-1)
    ...
    %========plotting for check========
%     figure()
%     subplot(331);plot(A);subplot(334);plot(Ac);subplot(337);plot(Apc);
%     subplot(332);plot(B);subplot(335);plot(Bc);subplot(338);plot(Bpc);
%     subplot(333);plot(C);subplot(336);plot(Cc);subplot(339);plot(Cpc);
end